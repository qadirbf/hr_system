module AttendanceHelper
  ...
  def late_count(employee_id, start_time, end_time)
    count = Array.new(2, 0)
    attends = Attendance.where("late>0 and employee_id=? and  (this_date>=to_date('#{start_time} 00:00:01') and this_date<=to_date1('#{end_time} 23:59:59'))", employee_id)
    ...
    forget_record_count = AttendLeave.count(:id, :conditions => ["absence_reason=14 and employee_id = ?
      and start_date between to_date('#{start_time} 00:00:01') and to_date('#{end_time} 23:59:59')", employee_id])
  ...
  def early_count(employee_id, start_time, end_time)
    count = Array.new(2, 0)
    attends = Attendance.where("early>0 and employee_id=? and  (this_date>=to_date('#{start_time} 00:00:01') and this_date<=to_date('#{end_time} 23:59:59'))", employee_id)
    attends.each do |attend|
      count[0] += 1
  ...
  def absence_count(employee_id, start_time, end_time)
    ...
    attends = Attendance.where
        ("attendances.employee_id=#{employee_id} and (this_date>=to_date('#{start_time} 00:00:00') and this_date<=to_date('#{end_time} 23:59:59'))
